    ...
    <div class="panel panel-default" data-example-id="togglable-tabs">
        ...
        <div class="panel-body">
            ...
            {{-- panel-body换成什么标签更好些?或者更好的排版? --}}
                <form action="{{URL('member/pay_accounts/payment')}}" method="POST" target="_blank" class="form-horizontal">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="payment" value="{{ $payAccount->payment }}">
                    <input type="hidden" name="quantity" value="1">
                    <input type="hidden" name="total_fee" value="{{ $payAccount->money }}">
                    <div class="form-group">
                        <div class="col-sm-6">
                            <button class="btn btn-lg btn-info">立即支付</button>
                        </div>
                    </div>
                </form>
        </div>
