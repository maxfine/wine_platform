...
+function ($) { "use strict";
    ...
    SidenavTree.prototype.applySearch = function() {
        ...
        //
        $('ul#side-menu > li.backnav', this.$el).each(function() {
            var $li = $(this)

            console.log($li[0])
            console.log($('span.nav-label', $li));
            console.log($('span.nav-label', $li).text());
            if (self.textContainsWords($('span.nav-label', $li).text(), words)) {
        ...
        $('ul#side-menu > li.backnav', this.$el).each(function() {
            ...
            //如果存在不hidden
            $li.toggleClass('hidden', !groupIsVisible)  //toggleClass('hidden', false), 如果包含则不隐藏
            if (groupIsVisible)
